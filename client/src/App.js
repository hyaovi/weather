...
class App extends Component {
  ...
  fetchWeather(event) {
    axios(`${PATH_BASE}${WEATHER_TYPE_REQUEST[0]}${CITY}${this.state.city}&${UNITS}&${MODE}&${APPID}`)
      .then(response => { this.setWeather(response.data) })
      .then(axios(`${PATH_BASE}${WEATHER_TYPE_REQUEST[1]}${CITY}${this.state.city}&${UNITS}&${MODE}&${APPID}`)
    ...
    return (
      ...
      <h1>Weather App</h1>
        <div className="App">
          ...
          {
            weather&&weatherforecast ? <WeatherDisplayer weather={weather} error={error} > <ForecastDisplayer weatherforecast={weatherforecast}/>
            </WeatherDisplayer> : <p></p>
          }
